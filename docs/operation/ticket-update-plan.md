...
> 2026-05-03 作成
> イン崩れ指数特化戦略に基づいて、BOA-99 を更新し、新規チケット BOA-100/101/102/103 を作成

---

## 既存チケット更新

### BOA-99: 出目分析テーブル構築と複数買い目表示（イン崩れ特化戦略）
...
- `docs/operation/ticket-update-plan.md` - このドキュメント
```
...
- 例: `[{ rank: 1, pattern: "4→2→3", probability: 12.5, recovery_rate: 138, reasoning: "..." }, ...]`

### 3. generate-predictions.js 更新
- generateRacePrediction() で generateVolatilePatterns() を呼び出し
- race.volatility.score >= 51 の時のみ実行
- 結果を predictions.volatile_patterns に格納

## テスト計画
- [ ] スコア < 51 のレースで null が返されること
- [ ] スコア >= 51 のレースで複数パターンが返されること
- [ ] パターンの probability（出現率）が降順になっていること
- [ ] 各パターンの reasoning が正常に生成されていること
...
### 4. スタイリング
- design-tokens.css の既存変数を使用
- モバイル対応（480px以下での表示確認）
...
- [ ] 複数パターンをタブで切り替えできること
- [ ] モバイル端末での表示が正常であること
- [ ] 既存の「複勝おすすめ買い目」との共存で UI が崩れないこと
...
### BOA-103: パターン実績統計テーブル（pattern_performance）
...
```
1. BOA-100: 出目分析テーブル構築 (1ヶ月)
   ↓ (完了後)
2. BOA-101: 買い目生成ロジック (2週間)
   ↓ (完了後)
3. BOA-102: フロントエンド UI (2週間) [並列で進行可能]
4. BOA-103: 実績統計テーブル (1ヶ月) [並列で進行可能]
```
...
### ドキュメント確認
- [ ] `docs/design/in-kuzure-specialization-strategy.md` を開発チームに共有
- [ ] このドキュメント (`docs/operation/ticket-update-plan.md`) を保存

### 関連チケット（別途検討）
- Phase 1: イン崩れ指数精度向上（会場別ルール拡充）
  - 新規チケット: BOA-105 を検討
